...
- 一套围绕 memory、pattern、skill、tool、session、runtime、events 的 **plugin execution model**
- 一组围绕执行策略和语义恢复的 **middle-protocol host**
- 一套围绕 `RunRequest`、`RunResult`、`RunUsage`、`RunArtifact`、`ExecutionContext` 的 **结构化 runtime contract**
...
- `ExecutionContext` 是单次 run 的状态载体
...
- [examples/production_coding_agent](examples/production_coding_agent)
  - production-style coding agent，展示 task packet、persistent memory、follow-up recovery 与 delivery artifacts
- [examples/multi_step_research](examples/multi_step_research)
  - 自定义 research pattern
...
开发者文档在 [docs-v2](docs-v2/README.md)。
